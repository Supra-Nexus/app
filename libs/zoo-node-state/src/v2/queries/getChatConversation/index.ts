import { ToolStatusType } from '@zooai/zoo-message-ts/api/general/types';
import {
  downloadFile,
  getJobFolderName,
  getLastMessagesWithBranches,
  getProviderFromJob,
} from '@zooai/zoo-message-ts/api/jobs/index';
import { type ChatMessage } from '@zooai/zoo-message-ts/api/jobs/types';
import { getZooFileProtocol } from '@zooai/zoo-message-ts/api/tools/index';
          ...
          if ('data' in response && '__created_files__' in response.data) {
            ...
            const fileResults = await Promise.all(
              filteredFiles.map(async (file) => {
                const response = await getZooFileProtocol(
                  nodeAddress,
                  token,
    ...
    try {
      const fileResults = await Promise.all(
        filePaths.map(async (filePath) => {
          const fullFilePath = `zoo://file/${filePath.replace('/main/', '/')}`;
          const response = await getZooFileProtocol(
            nodeAddress,
            token,

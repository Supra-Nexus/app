import { ToolStatusType } from '@supraai/supra-message-ts/api/general/types';
import {
  downloadFile,
  getJobFolderName,
  getLastMessagesWithBranches,
  getProviderFromJob,
} from '@supraai/supra-message-ts/api/jobs/index';
import { type ChatMessage } from '@supraai/supra-message-ts/api/jobs/types';
import { getSupraFileProtocol } from '@supraai/supra-message-ts/api/tools/index';
          ...
          if ('data' in response && '__created_files__' in response.data) {
            ...
            const fileResults = await Promise.all(
              filteredFiles.map(async (file) => {
                const response = await getSupraFileProtocol(
                  nodeAddress,
                  token,
    ...
    try {
      const fileResults = await Promise.all(
        filePaths.map(async (filePath) => {
          const fullFilePath = `supra://file/${filePath.replace('/main/', '/')}`;
          const response = await getSupraFileProtocol(
            nodeAddress,
            token,

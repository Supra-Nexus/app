...
pub struct ZooNodeProcessHandler {
    app: AppHandle,
...
impl ZooNodeProcessHandler {
    ...
    const PROCESS_NAME: &'static str = "zood";
    ...
    pub fn new(
        ...
        let options =
            ZooNodeOptions::with_app_options(app_resource_dir.clone(), app_data_dir.clone());
    ...
    async fn health(base_url: &str, timeout: Duration) -> bool {
        ...
        log::info!(
            "checking zood health of {} with timeout {:?}",
            url,
            timeout
        );
    ...
    async fn wait_zoo_node_server(&self) -> Result<(), String> {
        ...
        tokio::select! {
            _ = tokio::time::sleep(max_wait_time) => {
                ...
                Err(format!("wait zood server timeout after {}ms", elapsed.as_millis()))
            }
    ...
    pub fn set_options(&mut self, options: ZooNodeOptions) -> ZooNodeOptions {
        self.options = ZooNodeOptions::from_merge(self.options.clone(), options);
        self.options.clone()
    }
    ...
    pub fn set_default_options(&mut self) -> ZooNodeOptions {
        self.options = ZooNodeOptions::with_app_options(
    ...
    pub fn get_options(&self) -> ZooNodeOptions {
        self.options.clone()
    }

...
pub fn create_tray(app: &tauri::AppHandle) -> tauri::Result<()> {
    ...
    let zoo_spotlight_menu_item = MenuItemBuilder::with_id("zoo_spotlight", "Zoo Spotlight")
        .accelerator("CmdOrCtrl+Shift+J")
        .build(app)?;

    let open_zoo_node_manager_window_menu_item =
        MenuItemBuilder::with_id("open_zoo_node_manager_window", "Open").build(app)?;
    let zoo_node_manager_menu_item = SubmenuBuilder::new(app, "Zoo Node Manager")
        .item(&open_zoo_node_manager_window_menu_item)
        .build()?;

    let menu = MenuBuilder::new(app)
        .items(&[
            &show_menu_item,
            &zoo_node_manager_menu_item,
            &zoo_spotlight_menu_item,
            &quit_menu_item,
        ])
        .build()?;
    ...
    let _ = TrayIconBuilder::with_id("tray")
        ...
        .on_menu_event(move |tray, event| match event.id().as_ref() {
            ...
            "open_zoo_node_manager_window" => {
                recreate_window(tray.app_handle().clone(), Window::ZooNodeManager, true);
            }
            "zoo_spotlight" => {
            ...
            "quit" => {
                tauri::async_runtime::spawn(async move {
                    ...
                    if zoo_node_manager_guard.is_running().await {

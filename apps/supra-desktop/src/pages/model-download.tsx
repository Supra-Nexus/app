...
// You can fork these to HuggingFace as supra-ai/supra-nano-1, supra-ai/supra-eco-1, supra-ai/supra-coder-1
const SUPRA_MODELS = [
  {
...
export default function ModelDownloadPage() {
  ...
  const [selectedModel, setSelectedModel] = useState(SUPRA_MODELS[1].id); // Default to Supra Eco-1 (recommended)
  ...
  const ollamaConfig = {
    host: 'http://localhost:2000', // Supra node runs on port 2000
  };
  ...
  // Check if at least one Supra model is installed
  const hasInstalledModel = installedModels?.models?.some((model) =>
    SUPRA_MODELS.some((supraModel) => model.name.includes(supraModel.id.split(':')[0]))
  );
    ...
    const checkOllamaConnection = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.error('Supra node is not running on port 2000. Please ensure the Supra node is running.');
        }
      } catch (error) {
        console.error('Failed to connect to Supra node:', error);
        console.error('Cannot connect to Supra node on port 2000. The Supra node may not be running.');
      }
    };
  ...
  const handleDownload = async () => {
    ...
    // First check if Supra node is running
    try {
      const testResponse = await fetch(`${ollamaConfig.host}/api/tags`);
      if (!testResponse.ok) {
        console.error('Supra node is not responding. Please ensure it is running on port 2000');
        alert('Supra node is not running. Please start the Supra node and try again.');
        return;
      }
    } catch (error) {
      console.error('Cannot connect to Supra node:', error);
      alert('Cannot connect to Supra node on port 2000. Please ensure the Supra node is running.');
      return;
    }
    ...
    try {
      ...
      console.error('Make sure the Supra node is running on port 2000');
      
      // Show user-friendly error message
      const errorMessage = error?.message || 'Failed to download model';
      alert(`Error: ${errorMessage}\n\nPlease ensure the Supra node is running on port 2000.`);

...
import {
  ...
  localeOptions,
  useTranslation,
} from '@supraai/supra-i18n';
import { isSupraIdentityLocalhost } from '@supraai/supra-message-ts/utils/inbox_name_handler';
import { useSetMaxChatIterations } from '@supraai/supra-node-state/v2/mutations/setMaxChatIterations/useSetMaxChatIterations';
import { useUpdateNodeName } from '@supraai/supra-node-state/v2/mutations/updateNodeName/useUpdateNodeName';
import { useGetHealth } from '@supraai/supra-node-state/v2/queries/getHealth/useGetHealth';
import { useGetLLMProviders } from '@supraai/supra-node-state/v2/queries/getLLMProviders/useGetLLMProviders';
import { useGetPreferences } from '@supraai/supra-node-state/v2/queries/getPreferences/useGetPreferences';
import { useGetSupraFreeModelQuota } from '@supraai/supra-node-state/v2/queries/getSupraFreeModelQuota/useGetSupraFreeModelQuota';
import {
  Badge,
  Button,
  buttonVariants,
...
import { useDebounce } from '@supraai/supra-ui/hooks';
...
import {
  useSupraNodeGetOllamaVersionQuery,
  useSupraNodeRespawnMutation,
} from '../lib/supra-node-manager/supra-node-manager-client';
import { isHostingSupraNode } from '../lib/supra-node-manager/supra-node-manager-windows-utils';
import {
  useCheckUpdateQuery,
...
const SettingsPage = () => {
  ...
  const isLocalSupraNodeInUse = useSupraNodeManager(
    (state) => state.isInUse,
  );
  ...
  const form = useForm<FormSchemaType>({
    ...
    defaultValues: {
      ...
      nodeAddress: auth?.node_address,
      supraIdentity: auth?.supra_identity,
      ollamaVersion: '',
  ...
  const { data: ollamaVersion } = useSupraNodeGetOllamaVersionQuery();
  useEffect(() => {
  ...
  const { data: supraFreeModelQuota } = useGetSupraFreeModelQuota(
    { nodeAddress: auth?.node_address ?? '', token: auth?.api_v2_key ?? '' },
    { enabled: !!auth },
  );

  const { mutateAsync: respawnSupraNode } = useSupraNodeRespawnMutation();
  const { mutateAsync: updateNodeName, isPending: isUpdateNodeNamePending } =
    useUpdateNodeName({
      onSuccess: async () => {
        toast.success(t('settings.supraIdentity.success'));
        if (!auth) return;
        const newAuth: Auth = { ...auth };
        setAuth({
          ...newAuth,
          supra_identity: currentSupraIdentity,
        });
        if (isLocalSupraNodeInUse) {
          await respawnSupraNode();
        } else if (!isHostingSupraNode(auth.node_address)) {
          toast.info(t('supraNode.restartNode'));
        }
      },
      onError: (error) => {
        toast.error(t('settings.supraIdentity.error'), {
  ...
  useEffect(() => {
    if (isNodeInfoSuccess) {
      ...
      form.setValue('supraIdentity', nodeInfo?.node_name ?? '');
    }
  }, [form, isNodeInfoSuccess, nodeInfo?.node_name, nodeInfo?.version]);

  const currentSupraIdentity = useWatch({
    control: form.control,
    name: 'supraIdentity',
  });
  const handleUpdateNodeName = async () => {
    ...
    await updateNodeName({
      nodeAddress: auth?.node_address ?? '',
      token: auth?.api_v2_key ?? '',
      newNodeName: form.getValues().supraIdentity,
    });
  };
  ...
  const isIdentityLocalhost = isSupraIdentityLocalhost(
    auth?.supra_identity ?? '',
  );
      ...
      <div className="flex flex-col space-y-8 pr-2.5">
        <div className="flex flex-col space-y-8">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-text-default text-xs">
                        {supraFreeModelQuota.usedTokens.toLocaleString()}{' '}
                        <span className="text-text-tertiary text-xs">
                          / {supraFreeModelQuota.tokensQuota.toLocaleString()}
                        </span>
                      </span>
                      <Tooltip>
                        <TooltipTrigger>
                        ...
                        <TooltipContent>
                          <p>
                            ...
                            have approximately{' '}
                            {Math.floor(
                              (supraFreeModelQuota.tokensQuota -
                                supraFreeModelQuota.usedTokens) /
                                2,
                            )}{' '}
                            messages remaining.
                          </p>
                ...
                <CardFooter className="p-0">
                  <span className="text-text-default text-xs">
                    ...
                    {formatDuration(
                      intervalToDuration({
                        start: 0,
                        end: supraFreeModelQuota?.resetTime * 60 * 1000,
                      }),
                    )}
                  </span>
                </CardFooter>
          ...
          <div className="bg-bg-secondary space-y-4 rounded-lg p-4">
            <div>
              <h2 className="text-text-default text-base font-semibold">
                Supra Node Configuration
              </h2>
              <p className="text-text-secondary text-sm">
                Configure your Supra node connection and identity settings
              </p>
            </div>
            <Form {...form}>
              <form className="flex grow flex-col justify-between space-y-6 overflow-hidden">
                <div className="divide-divider flex flex-col divide-y">
                  {[
                    {
                      label: t('supraNode.nodeAddress'),
                      value: auth?.node_address,
                    },
                    {
                      label: t('supraNode.nodeVersion'),
                    ...
                    render={({ field }) => (
                      <TextField
                        field={{
                          ...
                          onKeyDown: (event) => {
                            if (
                              currentSupraIdentity === auth?.supra_identity
                            )
                              return;
                            if (event.key === 'Enter') {
                        ...
                        helperMessage={
                          <span className="flex items-center justify-start gap-3">
                            <span className="text-text-secondary hover:text-text-default inline-flex items-center gap-1 px-1 py-2.5">
                              {isIdentityLocalhost ? (
                                <a
                                  ...
                                  href={`https://supra-contracts.pages.dev?encryption_pk=${auth?.encryption_pk}&signature_pk=${auth?.identity_pk}&node_address=${auth?.node_address}`}
                                  rel="noreferrer"
                                  target="_blank"
                                >
                                  {t(
                                    'settings.supraIdentity.registerIdentity',
                                  )}
                                </a>
                              ) : (
                                <a
                                  ...
                                  href={`https://supra-contracts.pages.dev/identity/${auth?.supra_identity?.replace(
                                    '@@',
                                    '',
                                  )}`}
                                  rel="noreferrer"
                                ...
                                >
                                  {t(
                                    'settings.supraIdentity.goToSupraIdentity',
                                  )}
                                </a>
                              )}
                            ...
                            <a
                              ...
                              href="https://docs.supra.ngo/advanced/supra-identity-troubleshooting"
                              rel="noreferrer"
                              target="_blank"
                            >
                              {t(
                                'settings.supraIdentity.troubleRegisterIdentity',
                              )}
                            </a>
                          </span>
                        }
                        label={t('settings.supraIdentity.label')}
                      />
                    )}
                  />
                  {currentSupraIdentity !== auth?.supra_identity && (
                    <div className="space-y-1.5">
                      <p className="text-text-tertiary flex items-center gap-1 text-xs">
                        ...
                        {t('settings.supraIdentity.saveWillRestartApp')}
                      </p>
                    ...
                    >
                      ...
                      <span className="flex flex-col gap-0.5">
                        <span className="capitalize">
                          {t('settings.supraIdentity.checkIdentityInSync')}
                        </span>
                        <span className="text-text-tertiary">
                          {t(
                            'settings.supraIdentity.checkIdentityInSyncDescription',
                          )}
                        </span>
                      </span>
                    </a>

...
} from '@supraai/supra-node-state/forms/auth/quick-connection';
...
import { Button, buttonVariants, Checkbox } from '@supraai/supra-ui';
...
const TermsAndConditionsPage = () => {
  ...
  useEffect(() => {
    if (
      ...
      isLocalSupraNodeInUse
    ) {
      completeStep(OnboardingStep.TERMS_CONDITIONS, true);
    }
  }, [completeStep, isLocalSupraNodeInUse, termsAndConditionsAcceptedLegacy]);
  ...
  const setupDataForm = useForm<QuickConnectFormSchema>({
    ...
    defaultValues: {
      node_address: 'http://127.0.0.1:2000',
    },
  });
  ...
  const { isPending: supraNodeRemoveStorageIsPending } =
    useSupraNodeRemoveStorageMutation();
  const {
  ...
  const { isPending: supraNodeKillIsPending } = useSupraNodeKillMutation();
              ...
              components={{
                a: (
                  <a
                    ...
                    href={'https://www.supra.ai/terms-of-service'}
                    rel="noreferrer"
                    target={'_blank'}
                  />
                ),
                b: (
                  <a
                    className={'text-text-default underline'}
                    href={'https://www.supra.ai/privacy-policy'}
                    rel="noreferrer"
                    target={'_blank'}
                  />
                ),

...
import { decryptMessageWithPassphrase } from '@supraai/supra-message-ts/cryptography';
import {
  type RestoreConnectionFormSchema,
  restoreConnectionFormSchema,
} from '@supraai/supra-node-state/forms/settings/restore-connection';
import {
  Button,
  buttonVariants,
  ErrorMessage,
  ...
  const restore = async (values: RestoreConnectionFormSchema) => {
    try {
      ...
      if (decryptedValue) {
        ...
        // TODO: Add logic to test if setup data is valid to create an authenticated connection with Supra Node
        void navigate('/');
      }
    } catch {
      setError(true);

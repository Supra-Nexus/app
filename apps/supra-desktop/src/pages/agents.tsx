...
import { useExportAgent } from '@supraai/supra-node-state/v2/mutations/exportAgent/useExportAgent';
import { useRemoveAgent } from '@supraai/supra-node-state/v2/mutations/removeAgent/useRemoveAgent';
...
const SUPRA_DAPP_URL = 'https://supra-contracts.pages.dev';

export function AuthorAvatarLink({ author }: { author: string }) {
  const formattedAuthor = author
    .replace('@@', '')
    .replace('official.supra', 'official.sep-supra');

  return (
    <a
      href={`${SUPRA_DAPP_URL}/identity/${formattedAuthor}`}
    ...
    >
      <Avatar className={cn('h-5 w-5')}>
        {author === '@@official.supra' ? (
          <img alt="Supra" src={appIcon} />
        ) : (
              ...
              <div className="flex min-w-0 flex-1 flex-col">
                <CardTitle className="text-text-default truncate text-base leading-tight font-semibold">
                  <Link
                    ...
                    to={`https://store.supra.ngo/product/${agent.routerKey}`}
                    target="_blank"
                  >
                    {agent.name}
                  </Link>

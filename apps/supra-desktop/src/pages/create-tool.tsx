...
import { type CodeLanguage } from '@supraai/supra-message-ts/api/tools/types';
import { useOpenToolInCodeEditor } from '@supraai/supra-node-state/v2/mutations/openToolnCodeEditor/useOpenToolInCodeEditor';
import { useGetLLMProviders } from '@supraai/supra-node-state/v2/queries/getLLMProviders/useGetLLMProviders';
import { useGetToolProtocols } from '@supraai/supra-node-state/v2/queries/getToolProtocols/useGetToolProtocols';
import {
  Button,
  buttonVariants,
  Card,
  ...
  const modelOptions = useMemo(() => {
    ...
    const freeTrialModel = llmProviders?.find(
      (provider) =>
        ...
        SUPRA_FREE_TRIAL_MODEL_ID.toLowerCase(),
    );
    ...
    return [
      {
        ...
        name: t('tools.create.supraCodeGenerator'),
        placeholderId: 'code-generator',
        description: t('tools.create.supraCodeGeneratorDescription'),
      },
      {
        id: freeTrialModel?.id ?? '',
        name: t('tools.create.supraFreeTrial'),
        placeholderId: 'free-trial',
        description: t('tools.create.supraFreeTrialDescription'),
        recommendation: t('tools.create.supraFreeTrialRecommendation'),
      },
      {
        id: 'custom-model',
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  void openToolInCodeEditor({
                                    ...
                                    xSupraAppId,
                                    xSupraToolId,
                                    xSupraLLMProvider: defaulAgentId,
                                  });
                                }}
                                rounded="lg"
                                size="xs"

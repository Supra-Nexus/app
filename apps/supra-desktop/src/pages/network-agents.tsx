...
import { useAddNetworkTool } from '@supraai/supra-node-state/v2/mutations/addNetworkTool/useAddNetworkTool';
import { useGetAgents } from '@supraai/supra-node-state/v2/queries/getAgents/useGetAgents';
import { useGetInstalledNetworkTools } from '@supraai/supra-node-state/v2/queries/getInstalledNetworkTools/useGetInstalledNetworkTools';
import { type FormattedNetworkAgent } from '@supraai/supra-node-state/v2/queries/getNetworkAgents/types';
import { useGetNetworkAgents } from '@supraai/supra-node-state/v2/queries/getNetworkAgents/useGetNetworkAgents';
import { useGetToolsWithOfferings } from '@supraai/supra-node-state/v2/queries/getToolsWithOfferings/useGetToolsWithOfferings';
import { useGetWalletBalance } from '@supraai/supra-node-state/v2/queries/getWalletBalance/useGetWalletBalance';
import { useGetWalletList } from '@supraai/supra-node-state/v2/queries/getWalletList/useGetWalletList';
import {
  Card,
  CardContent,
  CardDescription,
...
export const MCP_SERVER_ID = 'supra-mcp-server';
...
const DiscoverNetworkAgents = ({
  ...
  const filteredAgents = (networkAgents ?? [])?.filter((agent) => {
    ...
    const isSupraProvider = agent.provider
      ?.toLowerCase()
      .includes('@@official.sep-supra');

    const matchesProvider =
      providerFilter === 'all' ||
      (providerFilter === 'supra' && isSupraProvider) ||
      (providerFilter === 'community' && !isSupraProvider);
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(value: 'all' | 'supra' | 'community') =>
    ...
    <Alert variant="warning" className="mb-6 border-0 bg-orange-300/5 p-6">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-text-secondary text-sm">
            Community agents are not verified by Supra. We may remove agents

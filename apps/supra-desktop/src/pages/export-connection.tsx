...
import { encryptMessageWithPassphrase } from '@supraai/supra-message-ts/cryptography';
import {
  type ExportConnectionFormSchema,
  exportConnectionFormSchema,
} from '@supraai/supra-node-state/forms/settings/export-connection';
import {
  Button,
  Form,
  FormField,
...
export const ExportConnection = () => {
  ...
  const download = async (): Promise<void> => {
    ...
    const path = await save({
      defaultPath: `${auth?.supra_identity}.supra.key`
        .replace(/@/g, '')
        .replace(/\//g, '_'),
    });
    if (path) {

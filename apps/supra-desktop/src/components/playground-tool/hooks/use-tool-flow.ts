...
import { type ToolMetadata } from '@supraai/supra-message-ts/api/tools/types';
import {
  buildInboxIdFromJobId,
  extractJobIdFromInbox,
} from '@supraai/supra-message-ts/utils/inbox_name_handler';
import { useCreateToolCode } from '@supraai/supra-node-state/v2/mutations/createToolCode/useCreateToolCode';
import { useCreateToolMetadata } from '@supraai/supra-node-state/v2/mutations/createToolMetadata/useCreateToolMetadata';
import { useExecuteToolCode } from '@supraai/supra-node-state/v2/mutations/executeToolCode/useExecuteToolCode';
import { useUpdateToolCodeImplementation } from '@supraai/supra-node-state/v2/mutations/updateToolCodeImplementation/useUpdateToolCodeImplementation';
...
export const useToolFlow = ({
  ...
  const generateMetadata = async () => {
    ...
    try {
      ...
      await createToolMetadata({
        ...
        jobId: extractJobIdFromInbox(chatInboxId),
        tools: currentTools,
        xSupraToolId: xSupraToolId, // Add tool ID to help server identify the context
      });
    } catch (error) {
  ...
  const executeToolCode: FormProps['onSubmit'] = useCallback(
    ...
    [executeToolCodeQuery, form, auth, toolCode, xSupraAppId, xSupraToolId],
  );

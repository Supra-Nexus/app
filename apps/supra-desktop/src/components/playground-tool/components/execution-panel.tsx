...
import { useUploadPlaygroundToolFiles } from '@supraai/supra-node-state/v2/mutations/uploadPlaygroundToolFiles/useUploadPlaygroundToolFiles';
import { useGetSupraFileProtocol } from '@supraai/supra-node-state/v2/queries/getSupraFileProtocol/useGetSupraFileProtocol';
import {
  Button,
  CommandShortcut,
  FileUploader,
...
const ToolLogsBase = ({
  ...
  const { data: logsFileBlob } = useGetSupraFileProtocol(
    {
  ...
  function processLogLines(lines: string[]) {
    return lines
      .filter((line) => {
        ...
        if (/<\/?supra-code-result>/.test(line)) return false;

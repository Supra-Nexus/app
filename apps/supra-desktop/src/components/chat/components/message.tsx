...
import { type MessageTrace } from '@supraai/supra-message-ts/api/jobs/types';
import { extractJobIdFromInbox } from '@supraai/supra-message-ts/utils';
import {
  type AssistantMessage,
  type FormattedMessage,
  type TextStatus,
} from '@supraai/supra-node-state/v2/queries/getChatConversation/types';
import { useGetMessageTraces } from '@supraai/supra-node-state/v2/queries/getMessageTraces/useGetMessageTraces';
...
export const MessageBase = ({
  ...
  const configDeepLinkMatcher = (content: string) => {
    const match = content.match(/supra:\/\/config\?tool=([^\s]+)/);
    return match ? match[1] : null;
  };

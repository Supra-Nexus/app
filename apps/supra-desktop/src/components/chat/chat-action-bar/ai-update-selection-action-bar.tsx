...
import { useUpdateAgentInJob } from '@supraai/supra-node-state/v2/mutations/updateAgentInJob/useUpdateAgentInJob';
import { useUpdateChatConfig } from '@supraai/supra-node-state/v2/mutations/updateChatConfig/useUpdateChatConfig';
...
import { AIAgentIcon } from '@supraai/supra-ui/assets';
...
import OLLAMA_MODELS_REPOSITORY from '../../../lib/supra-node-manager/ollama-models-repository.json';
...
const nonOllamaProviderModels = {
  'supra-backend:free_text_inference':
    'Supra AI model for text generation.',
  'supra-backend:code_generator':
    'Supra AI model for generating tool code.',
  'openai:gpt-4o':
...
export function AIModelSelectorBase({
  ...
  const isLocalSupraNodeIsUse = useSupraNodeManager(
    (state) => state.isInUse,
  );
              ...
              heading={
                <div className="flex items-center justify-between gap-2 pb-2">
                  ...
                  <Link
                    ...
                    to={
                      isLocalSupraNodeIsUse ? '/install-ai-models' : '/add-ai'
                    }
                  >
                    <PlusIcon className="size-4" />
                    {t('common.add')}

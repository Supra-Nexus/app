...
import { useDebounce, useMap } from '@supraai/supra-ui/hooks';
...
} from '../../..//lib/supra-node-manager/ollama-models';
import {
  useOllamaListQuery,
...
import {
  ...
  useSupraNodeGetOllamaApiUrlQuery,
...
export const OllamaModelsRepository = ({
  ...
  const columns = useMemo<ColumnDef<OllamaModelDefinition>[]>(
    () => [
      {
        accessorKey: 'name',
        header: t('supraNode.models.labels.models'),
        enableSorting: true,
        cell: (info) => {
      ...
      {
        accessorKey: 'tags',
        header: t('supraNode.models.labels.tags'),
        enableSorting: false,
        cell: (info) => {
          const model = info.row.original;
          return (

...
import { removeLLMProvider } from '@supraai/supra-node-state/v2/mutations/removeLLMProvider/index';
...
import { Button, Progress } from '@supraai/supra-ui';
import { useMap } from '@supraai/supra-ui/hooks';
...
import { useSupraNodeGetOllamaApiUrlQuery } from '../../../lib/supra-node-manager/supra-node-manager-client';
import { useAuth } from '../../../store/auth';
export const OllamaModelInstallButton = ({ model }: { model: string }) => {
  const { data: ollamaApiUrl } = useSupraNodeGetOllamaApiUrlQuery();
  ...
  const { mutateAsync: ollamaRemove } = useOllamaRemoveMutation(ollamaConfig, {
    onSuccess: (_, input) => {
      toast.success(
        t('supraNode.models.success.modelRemoved', {
    ...
    onError: (error, input) => {
      toast.error(
        t('supraNode.models.errors.modelRemoved', {
          modelName: input.model,
        }),
        {
    ...
    try {
      for await (const progress of progressIterator) {
        ...
        if (progress.status === 'success') {
          toast.success(
            t('supraNode.models.success.modelInstalled', {
              modelName: model,
            }),
          );
          if (auth) {
          ...
          toast.error(
            t('supraNode.models.errors.modelInstalled', {
              modelName: model,
            }),
          );
        }

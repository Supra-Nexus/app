...
import { useDuplicateTool } from '@supraai/supra-node-state/v2/mutations/duplicateTool/useDuplicateTool';
import { useExecuteToolCode } from '@supraai/supra-node-state/v2/mutations/executeToolCode/useExecuteToolCode';
import { useExportTool } from '@supraai/supra-node-state/v2/mutations/exportTool/useExportTool';
import { usePublishTool } from '@supraai/supra-node-state/v2/mutations/publishTool/usePublishTool';
import { useSetCommonToolsetConfig } from '@supraai/supra-node-state/v2/mutations/setCommonToolsetConfig/useSetCommonToolsetConfig';
import { useToggleEnableTool } from '@supraai/supra-node-state/v2/mutations/toggleEnableTool/useToggleEnableTool';
import { useUpdateTool } from '@supraai/supra-node-state/v2/mutations/updateTool/useUpdateTool';
import { useGetToolPlaygroundMetadata } from '@supraai/supra-node-state/v2/queries/getToolPlaygroundMetadata/useGetToolPlaygroundMetadata';
import { useGetToolsFromToolset } from '@supraai/supra-node-state/v2/queries/getToolsFromToolset/useGetToolsFromToolset';
import { useGetToolStoreDetails } from '@supraai/supra-node-state/v2/queries/getToolStoreDetails/useGetToolStoreDetails';
import {
  Alert,
  AlertDescription,
  AlertTitle,
...
import { ToolsIcon } from '@supraai/supra-ui/assets';
import { formatText } from '@supraai/supra-ui/helpers';
...
import { SUPRA_STORE_URL } from '../../../utils/store';
 ...
 */
function removeEmbeddingFields(tool: SupraTool): SupraTool {
  if (!tool) return tool;
...
export default function ToolDetailsCard({
  ...
  const {
    ...
    onSuccess: async (response) => {
      await open(
        `${SUPRA_STORE_URL}/store/revisions/complete?id=${response.response.revisionId}`,
      );
    },
    onError: (error) => {
  ...
  const performUpdateThisTool = async () => {
    ...
    await updateTool({
      ...
      toolType: toolType,
      toolPayload: { config: configToSave } as unknown as SupraTool,
      isToolEnabled: true,
      nodeAddress: auth?.node_address ?? '',
      token: auth?.api_v2_key ?? '',
    });
  ...
  const handleExecuteTool: FormProps['onSubmit'] = async (data) => {
    ...
    const payload = {
      ...
      xSupraAppId: 'supra-desktop',
      xSupraToolId: toolKey ?? '',
        ...
        <div className="flex w-full flex-col gap-6 md:flex-row">
          ...
          <div className="flex items-center gap-3">
            <div className="mb-4 flex items-center justify-between gap-4">
              ...
              {isPlaygroundTool &&
                'author' in tool &&
                tool.author === auth?.supra_identity && (
                  <Link
                    className={cn(
                      buttonVariants({
                        size: 'sm',
          ...
          <div className={cn(boxContainerClass, 'gap-7')}>
            {[
              ...
              toolStoreDetails?.product?.product?.category && {
                ...
                href: `${SUPRA_STORE_URL}/category/${toolStoreDetails?.product?.product?.category.id}`,
              },
              'author' in tool &&
                tool.author && {
                  label: 'Author',
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-text-secondary text-sm">
                      ...
                      <a
                        ...
                        href={SUPRA_STORE_URL}
                        rel="noreferrer"
                        target="_blank"
                      >
                        Supra AI Store
                      </a>{' '}
                      to make it available to all Supra users.
                    </p>
                  </div>
                    ...
                    <AlertDescription className="text-xs">
                      <p className="">
                        ...
                        <a
                          ...
                          href={`${SUPRA_STORE_URL}/store/revisions/complete?id=${publishToolData?.response.revisionId}`}
                          rel="noreferrer"
                          target="_blank"
                        >

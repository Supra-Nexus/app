...
import {
  type SupraTool,
  type SupraToolHeader,
  type SupraToolType,
} from '@supraai/supra-message-ts/api/tools/types';
import {
  buildInboxIdFromJobId,
  extractJobIdFromInbox,
} from '@supraai/supra-message-ts/utils/inbox_name_handler';
import {
  type UploadVRFilesFormSchema,
  uploadVRFilesFormSchema,
} from '@supraai/supra-node-state/forms/vector-fs/folder';
import { transformDataToTreeNodes } from '@supraai/supra-node-state/lib/utils/files';
import {
  DEFAULT_CHAT_CONFIG,
  FunctionKeyV2,
} from '@supraai/supra-node-state/v2/constants';
import { useCreateAgent } from '@supraai/supra-node-state/v2/mutations/createAgent/useCreateAgent';
import { useCreateJob } from '@supraai/supra-node-state/v2/mutations/createJob/useCreateJob';
import { useCreateRecurringTask } from '@supraai/supra-node-state/v2/mutations/createRecurringTask/useCreateRecurringTask';
import { useRemoveRecurringTask } from '@supraai/supra-node-state/v2/mutations/removeRecurringTask/useRemoveRecurringTask';
import { useRetryMessage } from '@supraai/supra-node-state/v2/mutations/retryMessage/useRetryMessage';
import { useSendMessageToJob } from '@supraai/supra-node-state/v2/mutations/sendMessageToJob/useSendMessageToJob';
import { useUpdateAgent } from '@supraai/supra-node-state/v2/mutations/updateAgent/useUpdateAgent';
import { useUploadVRFiles } from '@supraai/supra-node-state/v2/mutations/uploadVRFiles/useUploadVRFiles';
import { useGetAgent } from '@supraai/supra-node-state/v2/queries/getAgent/useGetAgent';
import { useGetListDirectoryContents } from '@supraai/supra-node-state/v2/queries/getDirectoryContents/useGetListDirectoryContents';
import { useGetAgentInboxes } from '@supraai/supra-node-state/v2/queries/getInboxes/useGetAgentInboxes';
import { useGetLLMProviders } from '@supraai/supra-node-state/v2/queries/getLLMProviders/useGetLLMProviders';
import { useGetSearchDirectoryContents } from '@supraai/supra-node-state/v2/queries/getSearchDirectoryContents/useGetSearchDirectoryContents';
import { useGetTool } from '@supraai/supra-node-state/v2/queries/getTool/useGetTool';
import { useGetTools } from '@supraai/supra-node-state/v2/queries/getToolsList/useGetToolsList';
import { useGetSearchTools } from '@supraai/supra-node-state/v2/queries/getToolsSearch/useGetToolsSearch';
import {
  Badge,
  Button,
  buttonVariants,
...
import {
  ...
  ScheduledTasksIcon,
  SendIcon,
} from '@supraai/supra-ui/assets';
import {
  formatDateToLocaleStringWithTime,
  formatText,
  getFileExt,
} from '@supraai/supra-ui/helpers';
...
function AgentForm({ mode }: AgentFormProps) {
  ...
  const submit = async (
    ...
    const agentData = {
      ...
      full_identity_name: `${auth.supra_identity}/main/agent/${agentIdToUse}`,
      llm_provider_id: values.llmProviderId,
      ui_description: values.uiDescription,
      storage_path: values.storage_path,
      knowledge: values.knowledge,
  ...
  const toolsRequiringConfig = useMemo(() => {
    const getToolsRequiringConfig = (tools: SupraToolHeader[]) => {
...
const ToolConfigModal = ({
  ...
  const tool = data?.content[0] as SupraTool;
  const isEnabled = data?.content[1] as boolean;
  const toolType = data?.type as SupraToolType;

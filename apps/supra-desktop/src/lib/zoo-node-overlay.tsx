...
import { Button } from '@supraai/supra-ui';
...
import { useDownloadTauriLogsMutation } from './supra-logs/logs-client';
import { useSupraNodeIsRunningQuery } from './supra-node-manager/supra-node-manager-client';
import { openSupraNodeManagerWindow } from './supra-node-manager/supra-node-manager-windows-utils';

export const SupraNodeRunningOverlay = ({
  children,
}: {
  children: React.ReactNode;
}) => {
  const auth = useAuth((store) => store.auth);
  const { data: isSupraNodeRunning, isPending: isSupraNodeRunningPending } =
    useSupraNodeIsRunningQuery();
  const isInUse = useSupraNodeManager((store) => store.isInUse);
  ...
  const isSupraNodeHealthy = isHealthSuccess && health.status === 'ok';

  if (isHealthPending || isSupraNodeRunningPending) {
    return (
      <div className="flex size-full flex-col items-center justify-center gap-3 py-8">
        <Loader2 className="h-8 w-8 animate-spin text-white" />
        <span className="text-text-secondary text-sm">
          Checking Supra Node Status ...
        </span>
      </div>
    );
  }
      ...
      <div className="flex size-full items-center justify-center">
        ...
        >
          <div className="space-y-2 text-center text-red-400">
            <p>Unable to connect to Supra Node.</p>
    ...
    <div className="flex h-screen flex-col items-center justify-center gap-10">
      <div className="mx-auto flex max-w-lg flex-col items-center gap-4">
        ...
        <h1 className="text-3xl font-bold">
          Unable to Connect to Supra Node
        </h1>
        <p className="text-text-secondary text-base">
          Please make sure the Supra Node is running and try again.
        </p>
      </div>
      {isInUse && !isSupraNodeRunning && (
        <Button
          onClick={async () => {

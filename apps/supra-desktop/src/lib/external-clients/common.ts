...
interface SupraNodeOptions {
  supra_tools_runner_deno_binary_path?: string;
  // other options...
}

export const getDenoBinPath = async (): Promise<string> => {
  try {
    const options = await invoke<SupraNodeOptions>('supra_node_get_options');
    const denoPath = options?.supra_tools_runner_deno_binary_path;
    if (!denoPath) {
    ...
    console.error('Failed to get Supra node options for Deno path:', error);

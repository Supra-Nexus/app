...
import { isLocalSupraNode } from '../lib/supra-node-manager/supra-node-manager-windows-utils';
import { type Auth, useAuth } from './auth';
import { useExperimental } from './experimental';
import { useSettings } from './settings';
import { useSupraNodeManager } from './supra-node-manager';
...
const stores = new Map(
  [useAuth, useSettings, useSupraNodeManager, useExperimental].map((s) => [
...
const handleAuthSideEffect = async (
  ...
  if (prevAuth && !auth) {
    ...
    useSupraNodeManager.getState().setIsInUse(false);
    void emit('rehydrate-storage', {
      triggeredBy: currentWindowLabel,
      stores: ['settings', 'supra-node-options'],
    });
    return;
  }

  if (!prevAuth) {
    const isLocal = isLocalSupraNode(auth?.node_address || '');
    const isRunning: boolean = await invoke('supra_node_is_running');
    void debug(`setting is in use isLocal:${isLocal} isRunning:${isRunning}`);
    useSupraNodeManager.getState().setIsInUse(isLocal && isRunning);
    void emit('rehydrate-storage', {
      triggeredBy: currentWindowLabel,
      stores: ['settings', 'supra-node-options'],
    });

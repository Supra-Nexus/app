import { FunctionKeyV2 } from '@supraai/supra-node-state/v2/constants';
import { useGetLLMProviders } from '@supraai/supra-node-state/v2/queries/getLLMProviders/useGetLLMProviders';
import { TooltipProvider } from '@supraai/supra-ui';
...
const ProtectedRoute = ({ children }: { children: React.ReactNode }) => {
  ...
  const supraNodeOptions = useSupraNodeManager(
    (state) => state.supraNodeOptions,
  );
  useSupraNodeEventsToast();
  const isInUse = useSupraNodeManager((state) => state.isInUse);
  const autoStartSupraNodeTried = useRef<boolean>(false);
  ...
  */
  useEffect(() => {
    void debug(
      `initializing autoStartSupraNodeTried.current:${autoStartSupraNodeTried.current} isInUse:${isInUse} supraNodeIsRunning:${supraNodeIsRunning}`,
    );
    if (
      !autoStartSupraNodeTried.current &&
      isInUse &&
      !supraNodeIsRunning
    ) {
      autoStartSupraNodeTried.current = true;
      void Promise.resolve().then(async () => {
        if (supraNodeOptions) {
          await supraNodeSetOptions(supraNodeOptions);
        }
  ...
  return <SupraNodeRunningOverlay>{children}</SupraNodeRunningOverlay>;
};

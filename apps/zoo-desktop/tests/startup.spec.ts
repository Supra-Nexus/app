...
test.describe('Zoo AI Desktop App Startup', () => {
  test.beforeAll(async () => {
    // Launch the Electron app
    const appPath = path.join(__dirname, '../src-tauri/target/debug/zoo-desktop');
  ...
  test('clicking Get Started Free should start Zoo Node', async () => {
    const getStartedButton = await page.locator('button:has-text("Get Started Free")').first();
    ...
    if (await getStartedButton.isVisible()) {
      ...
      if (error) {
        ...
        if (error.includes('Process ended before min time alive')) {
          console.error('zood process is failing to start - likely configuration issue');
      ...
      // Check if Zoo Node started
      await page.waitForTimeout(3000);
      
      // Look for success indicators
      const nodeRunning = await page.locator('text=/Zoo Node.*running/i').count();
  ...
  test('check zood process configuration', async () => {
    ...
    const config = await page.evaluate(async () => {
      try {
        ...
        if (invoke) {
          return await invoke('zoo_node_get_options');
        }
      } catch (e) {
    ...
    console.log('Zoo Node configuration:', JSON.stringify(config, null, 2));

...
import { isLocalZooNode } from '../lib/zoo-node-manager/zoo-node-manager-windows-utils';
import { type Auth, useAuth } from './auth';
import { useExperimental } from './experimental';
import { useSettings } from './settings';
import { useZooNodeManager } from './zoo-node-manager';
...
const stores = new Map(
  [useAuth, useSettings, useZooNodeManager, useExperimental].map((s) => [
...
const handleAuthSideEffect = async (
  ...
  if (prevAuth && !auth) {
    ...
    useZooNodeManager.getState().setIsInUse(false);
    void emit('rehydrate-storage', {
      triggeredBy: currentWindowLabel,
      stores: ['settings', 'zoo-node-options'],
    });
    return;
  }

  if (!prevAuth) {
    const isLocal = isLocalZooNode(auth?.node_address || '');
    const isRunning: boolean = await invoke('zoo_node_is_running');
    void debug(`setting is in use isLocal:${isLocal} isRunning:${isRunning}`);
    useZooNodeManager.getState().setIsInUse(isLocal && isRunning);
    void emit('rehydrate-storage', {
      triggeredBy: currentWindowLabel,
      stores: ['settings', 'zoo-node-options'],
    });

...
import {
  ...
  editAIModelSchema,
} from '@zooai/zoo-node-state/forms/agents/edit-ai';
import { Models } from '@zooai/zoo-node-state/lib/utils/models';
import { useRemoveLLMProvider } from '@zooai/zoo-node-state/v2/mutations/removeLLMProvider/useRemoveLLMProvider';
import { useUpdateLLMProvider } from '@zooai/zoo-node-state/v2/mutations/updateLLMProvider/useUpdateLLMProvider';
...
import { CreateAIIcon } from '@zooai/zoo-ui/assets';
...
import { useOllamaRemoveMutation } from '../lib/zoo-node-manager/ollama-client';
...
const AIsPage = () => {
  ...
  const isLocalZooNodeIsUse = useZooNodeManager(
    (state) => state.isInUse,
  );

  const onAddAgentClick = () => {
    if (isLocalZooNodeIsUse) {
  ...
  const submit = async (values: EditAIModelFormSchema) => {
    ...
    await updateLLMProvider({
      ...
      agent: {
        ...
        full_identity_name: `${auth.zoo_identity}/${auth.profile}/agent/${llmProviderId}`,
        id: llmProviderId,
        model,
        name: values.name,

...
import { type RecurringTask } from '@zooai/zoo-message-ts/api/recurring-tasks/types';
import { useExportAgent } from '@zooai/zoo-node-state/v2/mutations/exportAgent/useExportAgent';
import { useRemoveAgent } from '@zooai/zoo-node-state/v2/mutations/removeAgent/useRemoveAgent';
...
const ZOO_DAPP_URL = 'https://zoo-contracts.pages.dev';

export function AuthorAvatarLink({ author }: { author: string }) {
  const formattedAuthor = author
    .replace('@@', '')
    .replace('official.zoo', 'official.sep-zoo');

  return (
    <a
      href={`${ZOO_DAPP_URL}/identity/${formattedAuthor}`}
              ...
              <div className="flex min-w-0 flex-1 flex-col">
                <CardTitle className="text-text-default truncate text-base leading-tight font-semibold">
                  <Link
                    ...
                    to={`https://store.zoo.ngo/product/${agent.routerKey}`}
                    target="_blank"
                  >
                    {agent.name}
                  </Link>

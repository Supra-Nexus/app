...
import { encryptMessageWithPassphrase } from '@zooai/zoo-message-ts/cryptography';
import {
  type ExportConnectionFormSchema,
  exportConnectionFormSchema,
} from '@zooai/zoo-node-state/forms/settings/export-connection';
import {
  Button,
  Form,
  FormField,
...
export const ExportConnection = () => {
  ...
  const download = async (): Promise<void> => {
    ...
    const path = await save({
      defaultPath: `${auth?.zoo_identity}.zoo.key`
        .replace(/@/g, '')
        .replace(/\//g, '_'),
    });
    if (path) {

...
import { type CodeLanguage } from '@zooai/zoo-message-ts/api/tools/types';
import { useOpenToolInCodeEditor } from '@zooai/zoo-node-state/v2/mutations/openToolnCodeEditor/useOpenToolInCodeEditor';
import { useGetLLMProviders } from '@zooai/zoo-node-state/v2/queries/getLLMProviders/useGetLLMProviders';
import { useGetToolProtocols } from '@zooai/zoo-node-state/v2/queries/getToolProtocols/useGetToolProtocols';
import {
  Button,
  buttonVariants,
  Card,
...
import { SendIcon, ToolsIcon } from '@zooai/zoo-ui/assets';
  ...
  const modelOptions = useMemo(() => {
    ...
    const freeTrialModel = llmProviders?.find(
      (provider) =>
        ...
        ZOO_FREE_TRIAL_MODEL_ID.toLowerCase(),
    );
    ...
    return [
      {
        ...
        name: t('tools.create.zooCodeGenerator'),
        placeholderId: 'code-generator',
        description: t('tools.create.zooCodeGeneratorDescription'),
      },
      {
        id: freeTrialModel?.id ?? '',
        name: t('tools.create.zooFreeTrial'),
        placeholderId: 'free-trial',
        description: t('tools.create.zooFreeTrialDescription'),
        recommendation: t('tools.create.zooFreeTrialRecommendation'),
      },
      {
        id: 'custom-model',

...
import { decryptMessageWithPassphrase } from '@zooai/zoo-message-ts/cryptography';
import {
  type RestoreConnectionFormSchema,
  restoreConnectionFormSchema,
} from '@zooai/zoo-node-state/forms/settings/restore-connection';
import {
  Button,
  buttonVariants,
  ErrorMessage,
  ...
  const restore = async (values: RestoreConnectionFormSchema) => {
    try {
      ...
      if (decryptedValue) {
        ...
        // TODO: Add logic to test if setup data is valid to create an authenticated connection with Zoo Node
        void navigate('/');
      }
    } catch {
      setError(true);

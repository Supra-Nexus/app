...
import { type LLMProviderInterface } from '@zooai/zoo-message-ts/api/jobs/types';
import {
  addAiModelFormDefault,
  type AddAiModelFormSchema,
  addAiModelSchema,
} from '@zooai/zoo-node-state/forms/agents/add-ai';
import {
  Models,
  modelsConfig,
} from '@zooai/zoo-node-state/lib/utils/models';
import { useAddLLMProvider } from '@zooai/zoo-node-state/v2/mutations/addLLMProvider/useAddLLMProvider';
import { useScanOllamaModels } from '@zooai/zoo-node-state/v2/queries/scanOllamaModels/useScanOllamaModels';
import {
  Button,
  buttonVariants,
  Form,
...
const getGuideUrl = (model: Models) => {
  if (!model) {
    return 'https://docs.zoo.ngo/advanced/models';
  }

  const urlMap = {
    [Models.OpenAI]: 'https://docs.zoo.ngo/advanced/models/gpt',
    [Models.TogetherComputer]:
      'https://docs.zoo.ngo/advanced/models/together-ai',
    [Models.Ollama]: 'https://docs.zoo.ngo/advanced/models/ollama',
    [Models.Gemini]: 'https://docs.zoo.ngo/advanced/models/gemini',
    [Models.Groq]: 'https://docs.zoo.ngo/advanced/models/groq',
    [Models.OpenRouter]: 'https://docs.zoo.ngo/advanced/models/openrouter',
    [Models.Exo]: 'https://docs.zoo.ngo/advanced/models/exo',
    [Models.Claude]: 'https://docs.zoo.ngo/advanced/models/claude',
    [Models.DeepSeek]: 'https://docs.zoo.ngo/advanced/models/deepseek',
    [Models.Grok]: 'https://docs.zoo.ngo/advanced/models/grok',
  };

  return urlMap[model] || 'https://docs.zoo.ngo/advanced/models';
};
  ...
  const onSubmit = async (data: AddAiModelFormSchema) => {
    ...
    await addLLMProvider({
      ...
      agent: {
        ...
        full_identity_name: `${auth.zoo_identity}/${auth.profile}/agent/${generatedId}`,
        id: generatedId,
        model,
        name: data.name,
      },

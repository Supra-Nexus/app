...
import { useAddNetworkTool } from '@zooai/zoo-node-state/v2/mutations/addNetworkTool/useAddNetworkTool';
import { useGetAgents } from '@zooai/zoo-node-state/v2/queries/getAgents/useGetAgents';
import { useGetInstalledNetworkTools } from '@zooai/zoo-node-state/v2/queries/getInstalledNetworkTools/useGetInstalledNetworkTools';
import { type FormattedNetworkAgent } from '@zooai/zoo-node-state/v2/queries/getNetworkAgents/types';
import { useGetNetworkAgents } from '@zooai/zoo-node-state/v2/queries/getNetworkAgents/useGetNetworkAgents';
import { useGetToolsWithOfferings } from '@zooai/zoo-node-state/v2/queries/getToolsWithOfferings/useGetToolsWithOfferings';
import { useGetWalletBalance } from '@zooai/zoo-node-state/v2/queries/getWalletBalance/useGetWalletBalance';
import { useGetWalletList } from '@zooai/zoo-node-state/v2/queries/getWalletList/useGetWalletList';
import {
  Card,
  CardContent,
  CardDescription,
...
export const MCP_SERVER_ID = 'zoo-mcp-server';
...
const DiscoverNetworkAgents = ({
  ...
  const filteredAgents = (networkAgents ?? [])?.filter((agent) => {
    ...
    const isZooProvider = agent.provider
      ?.toLowerCase()
      .includes('@@official.sep-zoo');

    const matchesProvider =
      providerFilter === 'all' ||
      (providerFilter === 'zoo' && isZooProvider) ||
      (providerFilter === 'community' && !isZooProvider);
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(value: 'all' | 'zoo' | 'community') =>
    ...
    <Alert variant="warning" className="mb-6 border-0 bg-orange-300/5 p-6">
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-text-secondary text-sm">
            Community agents are not verified by Zoo. We may remove agents

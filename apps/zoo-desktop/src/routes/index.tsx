import { FunctionKeyV2 } from '@zooai/zoo-node-state/v2/constants';
import { useGetLLMProviders } from '@zooai/zoo-node-state/v2/queries/getLLMProviders/useGetLLMProviders';
import { TooltipProvider } from '@zooai/zoo-ui';
...
const ProtectedRoute = ({ children }: { children: React.ReactNode }) => {
  ...
  const zooNodeOptions = useZooNodeManager(
    (state) => state.zooNodeOptions,
  );
  useZooNodeEventsToast();
  const isInUse = useZooNodeManager((state) => state.isInUse);
  const autoStartZooNodeTried = useRef<boolean>(false);
  const { data: zooNodeIsRunning } = useZooNodeIsRunningQuery({
    refetchInterval: 1000,
  });
  const { mutateAsync: zooNodeSetOptions } =
    useZooNodeSetOptionsMutation();
  const { mutateAsync: zooNodeSpawn } = useZooNodeSpawnMutation();
  ...
  */
  useEffect(() => {
    void debug(
      `initializing autoStartZooNodeTried.current:${autoStartZooNodeTried.current} isInUse:${isInUse} zooNodeIsRunning:${zooNodeIsRunning}`,
    );
    if (
      !autoStartZooNodeTried.current &&
      isInUse &&
      !zooNodeIsRunning
    ) {
      autoStartZooNodeTried.current = true;
      void Promise.resolve().then(async () => {
        if (zooNodeOptions) {
          await zooNodeSetOptions(zooNodeOptions);
        }
        await zooNodeSpawn();
      });
    }
  ...
  return <ZooNodeRunningOverlay>{children}</ZooNodeRunningOverlay>;
};
...
export const useDefaultAgentByDefault = () => {
  ...
  const { data: defaultModel } = useZooNodeGetDefaultModel();

...
import { type MessageTrace } from '@zooai/zoo-message-ts/api/jobs/types';
import { extractJobIdFromInbox } from '@zooai/zoo-message-ts/utils';
import {
  type AssistantMessage,
  type FormattedMessage,
  type TextStatus,
} from '@zooai/zoo-node-state/v2/queries/getChatConversation/types';
import { useGetMessageTraces } from '@zooai/zoo-node-state/v2/queries/getMessageTraces/useGetMessageTraces';
...
export const MessageBase = ({
  ...
  const configDeepLinkMatcher = (content: string) => {
    const match = content.match(/zoo:\/\/config\?tool=([^\s]+)/);
    return match ? match[1] : null;
  };

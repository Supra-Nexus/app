...
import { useUploadPlaygroundToolFiles } from '@zooai/zoo-node-state/v2/mutations/uploadPlaygroundToolFiles/useUploadPlaygroundToolFiles';
import { useGetZooFileProtocol } from '@zooai/zoo-node-state/v2/queries/getZooFileProtocol/useGetZooFileProtocol';
import {
  Button,
  CommandShortcut,
  FileUploader,
...
const ToolLogsBase = ({
  ...
  const { data: logsFileBlob } = useGetZooFileProtocol(
    {
  ...
  function processLogLines(lines: string[]) {
    return lines
      .filter((line) => {
        ...
        if (/<\/?zoo-code-result>/.test(line)) return false;

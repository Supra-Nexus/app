...
import { type ToolMetadata } from '@zooai/zoo-message-ts/api/tools/types';
import {
  buildInboxIdFromJobId,
  extractJobIdFromInbox,
} from '@zooai/zoo-message-ts/utils/inbox_name_handler';
import { useCreateToolCode } from '@zooai/zoo-node-state/v2/mutations/createToolCode/useCreateToolCode';
import { useCreateToolMetadata } from '@zooai/zoo-node-state/v2/mutations/createToolMetadata/useCreateToolMetadata';
import { useExecuteToolCode } from '@zooai/zoo-node-state/v2/mutations/executeToolCode/useExecuteToolCode';
import { useUpdateToolCodeImplementation } from '@zooai/zoo-node-state/v2/mutations/updateToolCodeImplementation/useUpdateToolCodeImplementation';
...
export const useToolFlow = ({
  ...
  const xZooAppId = usePlaygroundStore((state) => state.xZooAppId);
  const xZooToolId = usePlaygroundStore((state) => state.xZooToolId);
  ...
  const generateMetadata = async () => {
    ...
    try {
      ...
      await createToolMetadata({
        ...
        jobId: extractJobIdFromInbox(chatInboxId),
        tools: currentTools,
        xZooToolId: xZooToolId, // Add tool ID to help server identify the context
      });
    } catch (error) {
  ...
  const executeToolCode: FormProps['onSubmit'] = useCallback(
    ...
    [executeToolCodeQuery, form, auth, toolCode, xZooAppId, xZooToolId],
  );

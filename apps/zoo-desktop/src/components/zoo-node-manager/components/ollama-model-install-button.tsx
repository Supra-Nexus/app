...
import { removeLLMProvider } from '@zooai/zoo-node-state/v2/mutations/removeLLMProvider/index';
...
import { Button, Progress } from '@zooai/zoo-ui';
...
import { useZooNodeGetOllamaApiUrlQuery } from '../../../lib/zoo-node-manager/zoo-node-manager-client';
import { useAuth } from '../../../store/auth';
export const OllamaModelInstallButton = ({ model }: { model: string }) => {
  const { data: ollamaApiUrl } = useZooNodeGetOllamaApiUrlQuery();
  ...
  const { mutateAsync: ollamaRemove } = useOllamaRemoveMutation(ollamaConfig, {
    onSuccess: (_, input) => {
      toast.success(
        t('zooNode.models.success.modelRemoved', {
    ...
    onError: (error, input) => {
      toast.error(
        t('zooNode.models.errors.modelRemoved', {
          modelName: input.model,
        }),
        {
    ...
    try {
      for await (const progress of progressIterator) {
        ...
        if (progress.status === 'success') {
          toast.success(
            t('zooNode.models.success.modelInstalled', {
              modelName: model,
            }),
          );
          if (auth) {
          ...
          toast.error(
            t('zooNode.models.errors.modelInstalled', {
              modelName: model,
            }),
          );
        }

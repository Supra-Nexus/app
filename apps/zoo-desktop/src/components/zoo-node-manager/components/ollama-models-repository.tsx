...
import { useDebounce, useMap } from '@zooai/zoo-ui/hooks';
...
} from '../../..//lib/zoo-node-manager/ollama-models';
import {
  useOllamaListQuery,
...
import {
  ...
  useZooNodeGetOllamaApiUrlQuery,
...
export const OllamaModelsRepository = ({
  ...
  const columns = useMemo<ColumnDef<OllamaModelDefinition>[]>(
    () => [
      {
        accessorKey: 'name',
        header: t('zooNode.models.labels.models'),
        enableSorting: true,
        cell: (info) => {
      ...
      {
        accessorKey: 'tags',
        header: t('zooNode.models.labels.tags'),
        enableSorting: false,
        cell: (info) => {
          const model = info.row.original;
          return (

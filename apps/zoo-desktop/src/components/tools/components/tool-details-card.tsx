...
import { useDuplicateTool } from '@zooai/zoo-node-state/v2/mutations/duplicateTool/useDuplicateTool';
import { useExecuteToolCode } from '@zooai/zoo-node-state/v2/mutations/executeToolCode/useExecuteToolCode';
import { useExportTool } from '@zooai/zoo-node-state/v2/mutations/exportTool/useExportTool';
import { usePublishTool } from '@zooai/zoo-node-state/v2/mutations/publishTool/usePublishTool';
import { useSetCommonToolsetConfig } from '@zooai/zoo-node-state/v2/mutations/setCommonToolsetConfig/useSetCommonToolsetConfig';
import { useToggleEnableTool } from '@zooai/zoo-node-state/v2/mutations/toggleEnableTool/useToggleEnableTool';
import { useUpdateTool } from '@zooai/zoo-node-state/v2/mutations/updateTool/useUpdateTool';
import { useGetToolPlaygroundMetadata } from '@zooai/zoo-node-state/v2/queries/getToolPlaygroundMetadata/useGetToolPlaygroundMetadata';
import { useGetToolsFromToolset } from '@zooai/zoo-node-state/v2/queries/getToolsFromToolset/useGetToolsFromToolset';
import { useGetToolStoreDetails } from '@zooai/zoo-node-state/v2/queries/getToolStoreDetails/useGetToolStoreDetails';
import {
  Alert,
  AlertDescription,
  AlertTitle,
...
import { ToolsIcon } from '@zooai/zoo-ui/assets';
import { formatText } from '@zooai/zoo-ui/helpers';
...
import { ZOO_STORE_URL } from '../../../utils/store';
 ...
 */
function removeEmbeddingFields(tool: ZooTool): ZooTool {
  if (!tool) return tool;
...
export default function ToolDetailsCard({
  ...
  const {
    ...
    onSuccess: async (response) => {
      await open(
        `${ZOO_STORE_URL}/store/revisions/complete?id=${response.response.revisionId}`,
      );
    },
    onError: (error) => {
  ...
  const performUpdateThisTool = async () => {
    ...
    await updateTool({
      ...
      toolType: toolType,
      toolPayload: { config: configToSave } as unknown as ZooTool,
      isToolEnabled: true,
      nodeAddress: auth?.node_address ?? '',
      token: auth?.api_v2_key ?? '',
    });
          ...
          <div className={cn(boxContainerClass, 'gap-7')}>
            {[
              ...
              toolStoreDetails?.product?.product?.category && {
                ...
                href: `${ZOO_STORE_URL}/category/${toolStoreDetails?.product?.product?.category.id}`,
              },
              'author' in tool &&
                tool.author && {
                  label: 'Author',
                ...
                <div className="flex flex-row items-center justify-between gap-7">
                  <div className="space-y-2">
                    ...
                    <p className="text-text-secondary text-sm">
                      ...
                      to make it available to all Zoo users.
                    </p>
                  </div>
                    ...
                    <AlertDescription className="text-xs">
                      <p className="">
                        ...
                        <a
                          ...
                          href={`${ZOO_STORE_URL}/store/revisions/complete?id=${publishToolData?.response.revisionId}`}
                          rel="noreferrer"
                          target="_blank"
                        >

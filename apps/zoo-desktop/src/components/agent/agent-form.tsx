...
} from '@zooai/zoo-message-ts/utils/inbox_name_handler';
import {
  type UploadVRFilesFormSchema,
  uploadVRFilesFormSchema,
} from '@zooai/zoo-node-state/forms/vector-fs/folder';
import { transformDataToTreeNodes } from '@zooai/zoo-node-state/lib/utils/files';
import {
  DEFAULT_CHAT_CONFIG,
  FunctionKeyV2,
} from '@zooai/zoo-node-state/v2/constants';
import { useCreateAgent } from '@zooai/zoo-node-state/v2/mutations/createAgent/useCreateAgent';
import { useCreateJob } from '@zooai/zoo-node-state/v2/mutations/createJob/useCreateJob';
import { useCreateRecurringTask } from '@zooai/zoo-node-state/v2/mutations/createRecurringTask/useCreateRecurringTask';
import { useRemoveRecurringTask } from '@zooai/zoo-node-state/v2/mutations/removeRecurringTask/useRemoveRecurringTask';
import { useRetryMessage } from '@zooai/zoo-node-state/v2/mutations/retryMessage/useRetryMessage';
import { useSendMessageToJob } from '@zooai/zoo-node-state/v2/mutations/sendMessageToJob/useSendMessageToJob';
import { useUpdateAgent } from '@zooai/zoo-node-state/v2/mutations/updateAgent/useUpdateAgent';
import { useUploadVRFiles } from '@zooai/zoo-node-state/v2/mutations/uploadVRFiles/useUploadVRFiles';
import { useGetAgent } from '@zooai/zoo-node-state/v2/queries/getAgent/useGetAgent';
import { useGetListDirectoryContents } from '@zooai/zoo-node-state/v2/queries/getDirectoryContents/useGetListDirectoryContents';
import { useGetAgentInboxes } from '@zooai/zoo-node-state/v2/queries/getInboxes/useGetAgentInboxes';
import { useGetLLMProviders } from '@zooai/zoo-node-state/v2/queries/getLLMProviders/useGetLLMProviders';
import { useGetSearchDirectoryContents } from '@zooai/zoo-node-state/v2/queries/getSearchDirectoryContents/useGetSearchDirectoryContents';
import { useGetTool } from '@zooai/zoo-node-state/v2/queries/getTool/useGetTool';
...
function AgentForm({ mode }: AgentFormProps) {
  ...
  const submit = async (
    ...
    const agentData = {
      ...
      full_identity_name: `${auth.zoo_identity}/main/agent/${agentIdToUse}`,
      llm_provider_id: values.llmProviderId,
      ui_description: values.uiDescription,
      storage_path: values.storage_path,
      knowledge: values.knowledge,
  ...
  const toolsRequiringConfig = useMemo(() => {
    const getToolsRequiringConfig = (tools: ZooToolHeader[]) => {
...
const ToolConfigModal = ({
  ...
  const tool = data?.content[0] as ZooTool;
  const isEnabled = data?.content[1] as boolean;
  const toolType = data?.type as ZooToolType;

...
interface ZooNodeOptions {
  zoo_tools_runner_deno_binary_path?: string;
  // other options...
}

export const getDenoBinPath = async (): Promise<string> => {
  try {
    const options = await invoke<ZooNodeOptions>('zoo_node_get_options');
    const denoPath = options?.zoo_tools_runner_deno_binary_path;
    if (!denoPath) {
    ...
    console.error('Failed to get Zoo node options for Deno path:', error);

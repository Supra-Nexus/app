...
// Mutations
export const useGalxeRegisterZooDesktopInstallationMutation = (
  ...
  return useMutation({
    mutationFn: async ({ address, signature, combined }): Promise<void> => {
      ...
      const baseUrl =
        appName === 'Zoo Desktop'
          ? 'https://backend-hosting.zoo.ngo'
          : 'https://dev-backend-hosting.zoo.ngo';
      await axios.post(`${baseUrl}/galxe/zoo-desktop-installation`, {
        address,
        signature,
        combined,
      });

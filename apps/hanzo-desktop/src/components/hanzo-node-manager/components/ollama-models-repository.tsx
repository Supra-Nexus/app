...
} from '../../..//lib/hanzo-node-manager/ollama-models';
import {
  useOllamaListQuery,
  useOllamaPullingQuery,
} from '../../../lib/hanzo-node-manager/ollama-client';
import {
  useHanzoNodeGetDefaultModel,
  useHanzoNodeGetOllamaApiUrlQuery,
...
export const OllamaModelsRepository = ({
  ...
  const columns = useMemo<ColumnDef<OllamaModelDefinition>[]>(
    () => [
      {
        accessorKey: 'name',
        header: t('hanzoNode.models.labels.models'),
        enableSorting: true,
        cell: (info) => {
      ...
      {
        accessorKey: 'tags',
        header: t('hanzoNode.models.labels.tags'),
        enableSorting: false,
        cell: (info) => {
          const model = info.row.original;
          return (
